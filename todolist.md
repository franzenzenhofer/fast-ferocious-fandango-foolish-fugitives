...
- Main loop in `src/main.ts` uses force-based drive + traction with fixed timestep.
...
- `tests/` is empty; deterministic RNG not wired yet.
...
## Next Fixes (Round 4)
- [ ] Introduce deterministic RNG hook for reproducible spawns.
- [ ] Add HUD telegraphs for police/heists and improve clarity at higher heat.
- [ ] Port traction + fixed timestep into `src/game/Game.ts` if switching to the modular path.
